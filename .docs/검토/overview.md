...
```
배터리 유효 영역: 1190 × 1960mm
메인보드 위치: 중앙 (595, 980)

각 모서리까지 대각선 거리:
d = √(595² + 980²) = √1,314,425 ≈ 1146mm

케이블 여유 (+20%): ~1.4m
```

| 카메라 | 위치 | 메인보드까지 거리 | 케이블 길이 |
|--------|------|------------------|-------------|
| CAM1 | (0, 0) | 1146mm | **1.4m** |
| CAM2 | (1190, 0) | 1146mm | **1.4m** |
| CAM3 | (0, 1960) | 1146mm | **1.4m** |
| CAM4 | (1190, 1960) | 1146mm | **1.4m** |

#### 7.1.2 I2C 케이블 평가

```
I2C 버스 최대 커패시턴스: 400pF

케이블 커패시턴스 (차폐 트위스트 페어):
├── 약 20pF/m
└── 1.4m × 4채널 = 약 112pF
...
| 항목 | 값 | 판정 |
|------|-----|------|
| 케이블 길이 | 1.4m × 4 (동일) | ✅ 대칭 구조 |
| 총 커패시턴스 | ~180pF | ✅ OK (400pF 이하) |
| I2C 버퍼 IC | 불필요 | ✅ 비용 절감 |
| 통신 속도 | 400kHz 유지 | ✅ |
...
| 타입 | 차폐 트위스트 페어 (STP) |
| 선경 | AWG 24 이상 |
| 구성 | 4선 (VCC, GND, SDA, SCL) |
| 커넥터 | JST-SH 4핀 (1.0mm pitch) |
| 커패시턴스 | < 25pF/m |
...
```
TCA9548A: 한 번에 하나의 채널만 활성화 (순차 읽기)
